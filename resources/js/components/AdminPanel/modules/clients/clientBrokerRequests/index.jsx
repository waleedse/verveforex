...
import { approveClientBrokerRequest, approveIbRequests, disapproveClientBrokerRequest, getAllIbRequests, getClientBrokerRequests } from '../../../../services/admin/commonService';
import { IMAGE_BASE_URL } from '../../../../config/constants';

const ClientBrokerRequests = () => {
    const [requests, setRequests] = useState([]);

    const getIbRequests = async () => {
        let res = await getClientBrokerRequests();
        setRequests(res?.clientbrokers);
    }
    ...
    const approveBroker = async (ib) => {
        toast.loading("Approving Broker Request...")
        let res = await approveClientBrokerRequest(ib?.id);
        toast.dismiss();
        if (res.status == 200) {
            toast.success("Broker request approved successfully")
            getIbRequests();
        } else {
    ...
    const declineBroker = async (ib) => {
        toast.loading("Declining Broker Request")
        let res = await disapproveClientBrokerRequest(ib?.id);
        toast.dismiss();
        if (res.status == 200) {
            toast.success("Broker request declined successfully")
            getIbRequests();
        } else {
            toast.error("Something went wrong")
        }
                                        ...
                                        requests && requests.map((item, index) => {
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td>{item?.broker?.title}</td>
                                                    <td>{item?.broker_signup_id}</td>
...
export default ClientBrokerRequests;
